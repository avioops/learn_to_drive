...
//
// import '../../Services/repo/question_repo.dart';
//
...
//
// class MockExam extends StatefulWidget {
//   const MockExam({Key? key}) : super(key: key);
//
//   @override
//   State<MockExam> createState() => _MockExamState();
// }
...
// class _MockExamState extends State<MockExam> {
//   List<Question> questions = [];
//   List<Question> attemptedQuestions = []; // Added attemptedQuestions list
//   int userScore = 0, currentQuestion = 1;
//   static const int startingSeconds = 0, startingMinutes = 30;
//   static int seconds = startingSeconds, minutes = startingMinutes;
//   static String digitSeconds = (startingSeconds >= 10) ? '$seconds' : '0$startingSeconds';
...
//
//   void getQuestion() {
//     QuestionRepo.getQuestion(
//       onSuccess: (question) {
//         setState(() {
//           questions = question..shuffle();
//           questions = questions.sublist(0, 25);
//           attemptedQuestions = questions; // Assign questions to attemptedQuestions
//         });
//       },
//       onError: (onError) {},
//     );
...
//                 onPressed: () {
//                   stopTimer();
//                   Get.to(() => MockExamAnswer(attemptedQuestions: [],));
//                 },
//               ),
//               TextButton(
//                 child: Text('try-again'.tr, style: const TextStyle(color: Colors.white),),
...
class _MockExamState extends State<MockExam> {
  ...
  void checkAnswer(String correctOption, String chosenOption, Question question) {
    setState(() {
      question.selectOption = chosenOption;
      if (correctOption == chosenOption) {
        userScore++;
      }
      if (currentQuestion == 25) {
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            TextButton(
              ...
              onPressed: () {
                stopTimer();
                Get.to(() => MockExamAnswer(attemptedQuestions: attemptedQuestions));
              },
            ),
            TextButton(
              child: Text('try-again'.tr, style: const TextStyle(color: Colors.white)),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 25),
                  Text("$currentQuestion out of 25"),
                  const SizedBox(height: 25),
                  Container(
                    decoration: const BoxDecoration(
                      color: Colors.white,
                  ...
                  QuestionOptionButtonWidget(
                    option: '${'A'.tr}. ${question.first.option1}',
                    onPressed: () {
                      checkAnswer(question.first.correctOption.toString(), 'A', question.first);
                    },
                  ),
                  QuestionOptionButtonWidget(
                    option: '${'B'.tr}. ${question.first.option2}',
                    onPressed: () {
                      checkAnswer(question.first.correctOption.toString(), 'B', question.first);
                    },
                  ),
                  QuestionOptionButtonWidget(
                    option: '${'C'.tr}. ${question.first.option3}',
                    onPressed: () {
                      checkAnswer(question.first.correctOption.toString(), 'C', question.first);
                    },
                  ),
                  QuestionOptionButtonWidget(
                    option: '${'D'.tr}. ${question.first.option4}',
                    onPressed: () {
                      checkAnswer(question.first.correctOption.toString(), 'D', question.first);
                    },
                  ),
                ],
              ),
