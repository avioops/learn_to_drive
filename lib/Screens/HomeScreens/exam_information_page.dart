...
import 'package:first_app/Controllers/exam_information_controller.dart';
import 'package:first_app/Models/exam_information_model.dart';
import 'package:first_app/utils/pdf_reader.dart'; // Import the PdfReaderScreen
import 'package:first_app/utils/widgets/screens_app_bar.dart';
import 'package:first_app/utils/widgets/button_widget.dart';
...
class _ExamInformationPageState extends State<ExamInformationPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() => (examInformationController.loading.value)
          ? const Center(child: CircularProgressIndicator())
          : SingleChildScrollView(
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    examInformationController.examInformations[index];
                    return Padding(
                      padding: const EdgeInsets.only(bottom: 10),
                      child: SizedBox(
                        height: 50,
                        ...
                        child: Row(
                          children: [
                            ListView.builder(
                                ...
                                itemCount: 1,
                                itemBuilder: (context, index) {
                                  Media media = examInformation
                                      .media![
                                  (chosenLanguage.value == 'English') ? 0 : 1];
                                  return IconButton(
                                    onPressed: () {
                                      Get.to(() => PdfReaderScreen(
                                          title: examInformation.name!,
                                          pdfFilename: media.originalUrl ?? ""));
                                    },
                                    icon: const Icon(Icons.picture_as_pdf),
                                    color: Colors.red,
                                    iconSize: 50,
